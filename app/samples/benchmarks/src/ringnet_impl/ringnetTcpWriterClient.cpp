...
RingnetTcpWriterClient::RingnetTcpWriterClient(ringnet::EventLoop &loop, size_t chunk_size)
	: connector(loop), packet(chunk_size, std::byte{ 'A' })
{
}

void RingnetTcpWriterClient::connect(std::string_view server_address, uint16_t server_port)
{
...
void RingnetTcpWriterClient::onConnected()
{
	assert(connection);
	...
	connection->onWrite([this](ringnet::events::WriteEvent &&) {
